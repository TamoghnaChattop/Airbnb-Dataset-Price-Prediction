# -*- coding: utf-8 -*-
"""
Created on Sat Dec  1 16:57:51 2018
...
for a in range (len(alphas)):
    ...
    for i in range(n_trials):
        ...
        mean_sq_train_lasso[a][i]=mse(y_pred_train_lasso,y_train)
        mean_sq_test_lasso[a][i]=mse(y_pred_test_lasso,y_test)
...
index_max_train=np.argmax(mean_accur_train_lasso)
index_max_test = np.argmax(mean_accur_test_lasso)
index_min_mean_sq_train=np.argmin(mean_mean_sq_train_lasso)
index_min_mean_sq_test=np.argmin(mean_mean_sq_test_lasso)

print('best R2 alpha:' ,alphas[index_max_test])
print('with mean R2 on test data :' ,mean_accur_test_lasso[index_max_test])
print('with mean  R2  on training data :', mean_accur_train_lasso[index_max_test])
print('lowest mean squared error alpha: ', alphas[index_min_mean_sq_test])
print('with mean squared error on test data :' ,mean_mean_sq_test_lasso[index_min_mean_sq_test])
print('with mean squared error  on training data :', mean_mean_sq_train_lasso[index_min_mean_sq_test])
...
Mdae_train_ridge=mdae(y_pred_train,y_train)
Mdae_test_ridge=mdae(y_pred_test,y_test)
...
print('MdAE train ridge', Mdae_train_ridge)
print('MdAE test ridge', Mdae_test_ridge)
...
df1 = df.drop(un_imp_features_lasso, axis=1)
...
plt.title('Linear Regression - Test dataset (Lasso Feature Selection)')
...
plt.ylabel('Target Predicted')
plt.xlabel('True Target')
plt.title('Linear Regression - Train dataset (Lasso Feature Selection)')
...
plt.xlabel('Price')
plt.title('AdaBoost Regression - Test dataset (Lasso Feature Selection)')
...
print('MSE train Adaboost lasso', result_training[1,1])
print('R2 train Adaboost lasso', result_training[1,0])
print('MAE train Adaboost lasso', result_training[1,2])
print('MdAE train Adaboost lasso', result_training[1,3])
plt.figure()
plt.plot(y_train_pred, 'r', label = 'pred') 
plt.plot(y_d.values, 'b', label = 'true')
plt.ylabel('Number of data points')
plt.xlabel('Price')
plt.title('AdaBoost Regression - Train dataset (Lasso Feature Selection)')
...
plt.ylabel('Number of data points')
plt.xlabel('Price')
plt.title('XGBoost Regression - Test dataset (Lasso Feature Selection)')
...
plt.ylabel('Number of data points')
plt.xlabel('Price')
plt.title('XGBoost Regression - Train dataset (Lasso Feature Selection)')
